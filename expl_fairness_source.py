...
# OUTPUT FILE RESULTS
sys.stdout = open("OUTPUT_OK_v1.txt", "w", buffering=1)
...
def compute_Rc(ratings_df, item_genres, target_genre):
    ...
    target_interactions = sum(
        1 for iid in ratings_df["item_id"] if target_genre in item_genres.get(iid, [])
    )
    return target_interactions / total_interactions

def fair_topk_rerank_debug(top_items, item_genres, K, target_genre, lambda_fair, dpf_mode=0, Rc=None):
    ...
    x = {i: pulp.LpVariable(f"x_{str(i)}", cat="Binary") for i, _ in top_items}

    if dpf_mode == 0:
        dpf_expr = (pulp.lpSum([x[i] for i, _ in top_items if i in I1]) / K -
                    pulp.lpSum([x[i] for i, _ in top_items if i in I2]) / K)
    elif dpf_mode == 1:
        if Rc is None:
            raise ValueError("Rc must be provided when dpf_mode == 1")
        dpf_expr = (pulp.lpSum([x[i] for i, _ in top_items if i in I1]) / K - Rc)
    elif dpf_mode == 2:
        dpf_expr = (pulp.lpSum([w[i] * x[i] for i, _ in top_items if i in I1]) / Z -
                    pulp.lpSum([w[i] * x[i] for i, _ in top_items if i in I2]) / Z)
    else:
        raise ValueError("Unknown dpf_mode")

    prob += ((1-lambda_fair)*pulp.lpSum([score * x[i] for i, score in top_items])/(K) - lambda_fair * FAIR_SCALE * dpf_expr)
    ...
    if dpf_mode == 0:
        dpf_val = (sum(1 for i in sel_items if i in I1) / K - sum(1 for i in sel_items if i in I2) / K)
    elif dpf_mode == 1:
        dpf_val = (sum(1 for i in sel_items if i in I1) / K - Rc)
    elif dpf_mode == 2:
        dpf_val = (sum(w[i] for i in sel_items if i in I1) / Z - sum(w[i] for i in sel_items if i in I2) / Z)
    ...
    for user in topn_pred[model_name]:
        ...
        for i, item_id in enumerate(original_rank):
            if item_id in rerank_rank1:
                delta = rerank_rank1.index(item_id) - i
            else:
                delta = +K
...
for K in K_all:
    ...
    # 3.1 LOADING RATINGS & MOVIES
    ratings = pd.read_csv(
        "data/HETREC/ratings.dat", sep="::", engine="python", encoding="latin-1",
        names=["user_id", "item_id", "rating", "timestamp"]
    )

    movies = pd.read_csv(
        "data/HETREC/movies.dat", sep="::", engine="python", encoding="latin-1",
    ...
    vaecf_split = RatioSplit(
        ...
        test_size=0.2, exclude_unknowns=True, seed=SEED
    )

    train_set = vaecf_split.train_set
    test_set = vaecf_split.test_set

    mf_model = MF(k=30, max_iter=50, learning_rate=0.01, lambda_reg=0.02, seed=SEED, verbose=True)
    ...
    for user_id, uidx in train_set.uid_map.items():
        ...
        items = [(train_set.item_ids[iidx], float(user_scores[iidx])) for iidx in top_indices]
            ...
            for lambda_fair in lambda_fairs:
                ...
                for model_name in ["VAECF"]:
                    for user, items in topn_pred_train["VAECF"].items():
                        ...
                        original = [i for i, _ in topn_pred_train["VAECF"][user][:K]]
                    ...
                    for user, items in topn_pred_test["VAECF"].items():
                        ...
                        original = [i for i, _ in topn_pred_test["VAECF"][user][:K]]
                ...
                # ------------------------
                # 3.4 ATTRIBUTE-LEVEL OUTCOME ANALYSIS
                # ------------------------
                item_attributes = {}
                ...
                df_train = build_rerank_dataset(topn_pred_train, reranked_train, item_attributes, K)
                if df_train.empty:
                    continue
                df_train = df_train[df_train["label"].isin([1, -1])]

                X_train = df_train.drop(columns=["item_id", "delta", "label", "num_genres", "__" + target_genre])
                ...
                df_test = build_rerank_dataset(topn_pred_test, reranked_test, item_attributes, K)
                if df_test.empty:
                    continue
                df_test = df_test[df_test["label"].isin([1, -1])]

                X_test = df_test.drop(columns=["item_id", "delta", "label", "num_genres", "__" + target_genre])
